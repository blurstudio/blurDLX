...
void		BlurBitmap::boundaryFill( int x, int y, AColor boundary, AColor fill ) {
	...
	if (!this->push(x,y)) return;

	while( this->pop(x,y) ) {
		y1 = y;
		while ( y1 >= 0 && !this->isWithinThreshold( this->pixelAtPoint( x, y1, tempPtr ), boundary ) && !this->isWithinThreshold( this->pixelAtPoint( x, y1, tempPtr ), fill ) ) y1--;
		y1++;
		spanLeft = spanRight = 0;
		AColor pixel;
		while ( y1 < this->height && !this->isWithinThreshold( this->pixelAtPoint( x,y1, tempPtr ), boundary ) && !this->isWithinThreshold( this->pixelAtPoint( x,y1, tempPtr ), fill ) ) {
			this->setPixelAtPoint( x, y1, fill, tempPtr );

			pixel = this->pixelAtPoint( x-1, y1 );
			if ( !spanLeft && x > 0 && !this->isWithinThreshold( pixel, boundary ) && !this->isWithinThreshold( pixel, fill ) ) {
				if ( !this->push( x-1, y1 ) ) return;
				spanLeft = 1;
			}
			else if ( spanLeft && x > 0 && this->isWithinThreshold( pixel, boundary ) || this->isWithinThreshold( pixel, fill ) ) {
				spanLeft = 0;
			}

			pixel = this->pixelAtPoint( x+1, y1 );
			if ( !spanRight && x < this->width - 1 && !this->isWithinThreshold( pixel, boundary ) && !this->isWithinThreshold( pixel, fill ) ) {
				if ( !(this->push( x+1, y1))) return;
				spanRight = 1;
			}
			else if ( spanRight && x < this->width - 1 && this->isWithinThreshold( pixel, boundary ) || this->isWithinThreshold( pixel, fill ) ) {
				spanRight = 0;
			}
			y1++;
		}
...
float		BlurBitmap::distance( Point2 a, Point2 b ) { return sqrt( pow( ( b.x - a.x ), 2 ) + pow( ( b.y - a.y ), 2 ) ); }
void		BlurBitmap::drawArea( Array* points, AColor clr ) {
	for ( int i = 1; i < points->size; i++ )
		this->drawLine( points->data[i-1]->to_point2(), points->data[i]->to_point2(), clr );
	this->drawLine( points->data[ points->size - 1 ]->to_point2(), points->data[0]->to_point2(), clr );
}
Point2		BlurBitmap::drawFilledArea( Array* points, AColor boundary, AColor fill ) {
	...
	if ( points->size > 0 ) {
		...
		for ( int i = 1; i < points->size; i++ ) {
			...
			xTotal += center.x;
			yTotal += center.y;
			this->drawLine( center, points->data[i]->to_point2(), boundary );
		}
		center = points->data[ points->size - 1 ]->to_point2();
		xTotal += center.x;
		yTotal += center.y;
		this->drawLine( center, points->data[0]->to_point2(), boundary );

		// Now fill with desired color
		float size = points->size;
		center.x = floor( xTotal / size );
		center.y = floor( yTotal / size );
		this->boundaryFill( (int) center.x, (int) center.y, boundary, fill );
	}
	return center;
}
void		BlurBitmap::drawLine( Point2 startPoint, Point2 endPoint, AColor clr ) { this->drawLine( startPoint.x, startPoint.y, endPoint.x, endPoint.y, clr ); }
void		BlurBitmap::drawLine( float startX, float startY, float endX, float endY, AColor clr ) {
	...
	this->source->PutPixels( startX, startY, 1, lineColor );
	if ( abs( dx ) > abs( dy ) ) {
		float m = dy/dx;
		t += startY;
		dx = ( dx < 0 ) ? -1 : 1;
		...
		while ( currX != endX ) {
			currX += dx;
			t += m;
			this->source->PutPixels( currX, (int) t, 1, lineColor );
		}
	}
	else if ( dy != 0 ) {
		float m = dx/dy;
		...
		while ( currY != endY ) {
			currY += dy;
			t += m;
			this->source->PutPixels( (int) t, currY, 1, lineColor );
		}
	}
	free( lineColor );
}
void		BlurBitmap::drawGradientTriangle( Point2 a, Point2 b, Point2 c, AColor colorA, AColor colorB, AColor colorC, int mode ) {
	...
	for ( int y = minY; y <= maxY; y++ ) {
		...
		for ( int x = 0; x < rowWidth; x++ ) {
			currPoint.x = minX + x;
			currPoint.y = y;
			if ( this->isPointInTriangle( currPoint, a, b, c ) ) {
				AColor mixColor = this->mixTriangleColor( currPoint, a, b, c, colorA, colorB, colorC );
				if ( mode == SCREEN ) {
		...
		this->source->PutPixels( minX, y, rowWidth, row );
	}
	free( row );
}
...
void		BlurBitmap::fill( AColor clr ) {
	...
	color.b = clr.b;
	color.a = clr.a;
	if ( ( line = (BMM_Color_fl* )calloc(this->width, sizeof(BMM_Color_fl)))) {
		int ix, iy;
		for ( ix = 0, lp = line; ix < this->width; ix++, lp++ )
			*lp = color;
		for ( iy = 0; iy < this->height; iy++ )
			this->source->PutPixels( 0, iy, this->width, line );
		free( line );
	}
}
void		BlurBitmap::floodFill( int x, int y, AColor old, AColor fill ) {
	if ( this->isWithinThreshold( old, fill ) ) return;
	...
	while( pop(x,y) ) {
		y1 = y;
		while ( y1 >= 0 && this->isWithinThreshold( this->pixelAtPoint( x, y1, tempPtr ), old ) ) y1--;
		y1++;
		spanLeft = spanRight = 0;
		AColor pixel;
		while ( y1 < this->height && this->isWithinThreshold( this->pixelAtPoint( x,y1, tempPtr ), old ) ) {
			this->setPixelAtPoint( x, y1, fill, tempPtr );
			pixel = this->pixelAtPoint( x-1, y1 );

			if ( !spanLeft && x > 0 && this->isWithinThreshold( pixel, old ) ) {
				if ( !push( x-1, y1 ) ) return;
				spanLeft = 1;
			}
			else if ( spanLeft && x > 0 && !this->isWithinThreshold( pixel, old ) ) {
				spanLeft = 0;
			}

			pixel = this->pixelAtPoint( x+1, y1 );
			if ( !spanRight && x < this->width - 1 && this->isWithinThreshold( pixel, old ) ) {
				if ( !(push( x+1, y1))) return;
				spanRight = 1;
			}
			else if ( spanRight && x < this->width - 1 && !this->isWithinThreshold( pixel, old ) ) {
				spanRight = 0;
			}
			y1++;
		}
...
bool		BlurBitmap::hasPoint( Point2 point ) { return this->hasPoint( point.x, point.y ); }
...
bool		BlurBitmap::isPointInTriangle( Point2 point, Point2 a, Point2 b, Point2 c ) {
	float areaOne		= this->triangleArea( point, b, c );
	float areaTwo		= this->triangleArea( point, a, c );
	float areaThree		= this->triangleArea( point, a, b );
	float areaTriangle	= this->triangleArea( a, b, c );
...
bool		BlurBitmap::isWithinThreshold( AColor clr, BMM_Color_fl* colorPtr ) { return this->isWithinThreshold( clr.r, clr.g, clr.b, colorPtr->r, colorPtr->g, colorPtr->b ); }
bool		BlurBitmap::isWithinThreshold( AColor clr, AColor clr2 )			{ return this->isWithinThreshold( clr.r, clr.g, clr.b, clr2.r, clr2.g, clr2.b ); }
AColor		BlurBitmap::mixTriangleColor( Point2 p, Point2 a, Point2 b, Point2 c, AColor colorA, AColor colorB, AColor colorC ) {
	float PA	= this->distance( p, a );
	float PB	= this->distance( p, b );
	float PC	= this->distance( p, c );
	float total	= PA + PB + PC;

	if ( PA == 0 )		{ return colorA; }
	else if ( PB == 0 )	{ return colorB; }
	else if ( PC == 0 )	{ return colorC; }
	else {
		AColor outColor;
...
AColor		BlurBitmap::pixelAtPoint( int x, int y, BMM_Color_fl* colorPtr ) {
	...
	if ( this->hasPoint( x, y ) ) {
		...
		this->source->GetPixels( x, y, 1, colorPtr );
...
bool		BlurBitmap::pop( int &x, int &y ) {
	if ( this->stackPointer > 0 ) {
		int p = this->stack[ this->stackPointer ];
		x = p / this->height;
		y = p % this->height;
		stackPointer--;
		return 1;
...
bool		BlurBitmap::push( int x, int y ) {
	if ( this->stackPointer < STACK_SIZE - 1 ) {
		this->stackPointer++;
		this->stack[ this->stackPointer ] = this->height * x + y;
		return 1;
	}
	else return 0;
}
bool		BlurBitmap::setPixelAtPoint( int x, int y, BMM_Color_fl* colorPtr, int mode ) {
	if ( this->hasPoint( x,y ) ) {
		if ( mode == SCREEN ) {
			...
			this->source->GetPixels( x, y, 1, temp );
			if ( temp->r < colorPtr->r ) {
				if ( temp->g < colorPtr->g ) {
					if ( temp->b < colorPtr-> b ) {
						this->source->PutPixels( x, y, 1, colorPtr );
						return true;
					}
				}
			}
...
bool		BlurBitmap::setPixelAtPoint( int x, int y, AColor clr, BMM_Color_fl* colorPtr, int mode ) {
	...
	if ( this->hasPoint( x,y ) ) {
		...
		success = this->setPixelAtPoint( x, y, colorPtr, mode );
...
Value*		BlurBitmap::get_property( Value** arg_list, int count ) {
	...
	if		( key == n_bitmap )					vl.result = new MAXBitMap( this->info, this->source );
	else if ( key == n_boundaryFill )			vl.result = NEW_GENERIC_METHOD( boundaryFill );
	else if ( key == n_drawArea )				vl.result = NEW_GENERIC_METHOD( drawArea );
	else if ( key == n_drawFilledArea )			vl.result = NEW_GENERIC_METHOD( drawFilledArea );
	else if ( key == n_drawLine )				vl.result = NEW_GENERIC_METHOD( drawLine );
	else if ( key == n_drawGradientTriangle )	vl.result = NEW_GENERIC_METHOD( drawGradientTriangle );
	else if ( key == n_drawTriangle )			vl.result = NEW_GENERIC_METHOD( drawTriangle );
	else if ( key == n_fill )					vl.result = NEW_GENERIC_METHOD( fill );
	else if ( key == n_floodFill )				vl.result = NEW_GENERIC_METHOD( floodFill );
	else if ( key == n_hasPoint )				vl.result = NEW_GENERIC_METHOD( hasPoint );
	else if ( key == n_height )					vl.result = Integer::intern( this->height );
	else if ( key == n_pixelAtPoint )			vl.result = NEW_GENERIC_METHOD( pixelAtPoint );
	else if ( key == n_saveToFile )				vl.result = NEW_GENERIC_METHOD( saveToFile );
	else if	( key == n_show )					vl.result = NEW_GENERIC_METHOD( show );
	else if ( key == n_setPixelAtPoint )		vl.result = NEW_GENERIC_METHOD( setPixelAtPoint );
	else if ( key == n_width )					vl.result = Integer::intern( this->width );
	else										throw RuntimeError( key->to_string(), " is not a member of BlurBitmap" );
...
Value*		BlurBitmap::applyMethod( Value* methodID, Value** arg_list, int count, CallContext* cc ) {
	...
	if ( methodID == n_boundaryFill ) {
		...
		if ( this->hasPoint( x, y ) ) {
			this->boundaryFill( x, y, arg_list[1]->to_acolor(), arg_list[2]->to_acolor() );
		}
	}
		...
		this->drawArea( (Array*) arg_list[0]->eval(), arg_list[1]->eval()->to_acolor() );
	}
	else if ( methodID == n_drawFilledArea ) {
		check_arg_count( drawFilledArea, 3, count );
		Point2 center	= this->drawFilledArea( (Array*) arg_list[0]->eval(), arg_list[1]->eval()->to_acolor(), arg_list[2]->eval()->to_acolor() );
		vl.result		= new Point2Value( center );
	}
	else if ( methodID == n_drawLine ) {
		check_arg_count( drawLine, 3, count );
		this->drawLine( arg_list[0]->eval()->to_point2(), arg_list[1]->eval()->to_point2(), arg_list[2]->eval()->to_acolor() );
	}
		...
		this->drawGradientTriangle( arg_list[0]->eval()->to_point2(), arg_list[1]->eval()->to_point2(), arg_list[2]->eval()->to_point2(), arg_list[3]->eval()->to_acolor(), arg_list[4]->eval()->to_acolor(), arg_list[5]->eval()->to_acolor(), mode );
	}
		...
		this->drawTriangle( arg_list[0]->eval()->to_point2(), arg_list[1]->eval()->to_point2(), arg_list[2]->eval()->to_point2(), arg_list[3]->to_acolor(), mode );
	}
	else if ( methodID == n_fill ) {
		check_arg_count( fill, 1, count );
		this->fill( arg_list[0]->to_acolor() );
	}
		...
		int y = arg_list[0]->to_point2().y;
		if ( this->hasPoint( x, y ) ) {
			this->floodFill( x, y, this->pixelAtPoint( x,y ), arg_list[1]->to_acolor() );
		}
	}
	else if ( methodID == n_hasPoint ) {
		check_arg_count( hasPoint, 1, count );
		vl.result = ( this->hasPoint( arg_list[0]->eval()->to_point2() ) ) ? &true_value : &false_value;
	}
	else if ( methodID == n_pixelAtPoint ) { 
		check_arg_count( pixelAtPoint, 1, count );
		vl.result = ColorValue::intern( this->pixelAtPoint( arg_list[0]->eval()->to_point2().x, arg_list[0]->eval()->to_point2().y ) );
	}
		...
		this->info.SetName( arg_list[0]->to_string() );
		this->source->OpenOutput( &this->info );
		this->source->Write( &this->info, frame );
		this->source->Close( &this->info );
	}
	else if ( methodID == n_setPixelAtPoint ) { 
		check_arg_count( setPixelAtPoint, 2, count );
		vl.result = ( this->setPixelAtPoint( arg_list[0]->eval()->to_point2().x, arg_list[0]->eval()->to_point2().y, arg_list[1]->to_acolor() ) ) ? &true_value : &false_value;
	}
	return_value( vl.result );
}
...
void BlurBitmapInit() { mprintf( "------ blurBitmap Loaded - Version:%.2f ------\n", ((float) BLURBITMAP_VERSION) / 1000.0f ); }